...
class PawnTest {
    ...
    @Test
    @DisplayName("폰의 대각선 위치에 다른 팀 킹이 있는 경우 CATCH_KING을 반환하는지 검증")
    void catchKing() {
        Pawn pawn = new Pawn(D4, WHITE);
        PiecesOnChessBoard piecesOnChessBoard = new PiecesOnChessBoard(List.of(new King(C5, BLACK)));
        Assertions.assertThat(pawn.move(C5, piecesOnChessBoard))
                .isEqualTo(CATCH_KING);
    }

    @Test
    @DisplayName("폰의 대각선 위치에 같은 팀 킹이 있는 경우 이동이 불가능한지 검증")
    void moveFailureByOurKing() {
        Pawn pawn = new Pawn(D4, WHITE);
        PiecesOnChessBoard piecesOnChessBoard = new PiecesOnChessBoard(List.of(new King(C5, WHITE)));
        Assertions.assertThat(pawn.move(A4, piecesOnChessBoard))
                .isEqualTo(FAILURE);
    }
}
